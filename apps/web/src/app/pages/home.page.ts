    ...
    <section class="page page-home memory-home">
      <header class="page-header home-dashboard-hero">
        <div>
          ...
          <h1>Your organization already knows how to do more.</h1>
          <p>Find proven AI capabilities, preserve their evidence and context, and transfer what works without losing authorship or governance.</p>
        </div>
      ...
      @if (loading()) {
        ...
        <div class="home-widget-grid">
          ...
          <article class="home-widget continuity-widget" [class.passed]="departure()?.passed">
            <header><div><span class="eyebrow">Continuity proof</span><h2>Mai left. The capability stayed.</h2></div><span class="continuity-state">{{ departure()?.passed ? 'Verified' : 'Review' }}</span></header>
            <p>The original author remains credited while Dara inherits stewardship and a governed path to reuse.</p>
        ...
        <section class="home-widget action-widget">
          ...
          <div class="home-action-grid">
            ...
            <a routerLink="/memory/recommendations"><span>◎</span><div><strong>Recommend for a task</strong><small>Describe the work and surface relevant organizational practice.</small></div><i>→</i></a>
